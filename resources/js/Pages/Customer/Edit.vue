...
const form = useForm({
    ...
    bandwidth: props.customer.bandwidth || '', lat: props.customer.lat || '', lng: props.customer.lng || '',
    status: props.customer.status, notes: props.customer.notes || '',
    // ONT fields
    ont_name: ont.name || '', ont_serial_number: ont.serial_number || '',
    odp_id: ont.odp_id || '', olt_id: ont.olt_id || '',
    pon_port_id: ont.pon_port_id || '', ont_id_number: ont.ont_id_number || '',
});
            ...
            <form @submit.prevent="submit" class="space-y-6 rounded-lg bg-white p-6 shadow">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <div><label class="block text-sm font-medium text-gray-700">Nama *</label><input v-model="form.name" type="text" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm sm:text-sm" /></div>
                        ...
                        <div class="sm:col-span-2"><label class="block text-sm font-medium text-gray-700">Alamat *</label><input v-model="form.address" type="text" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm sm:text-sm" /></div>
