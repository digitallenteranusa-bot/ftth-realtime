...
const props = defineProps({ odps: Array, customers: Array, olts: Array, ponPorts: Array });
...
watch(() => form.customer_id, (id) => {
    ...
    if (!customer) return;
    const ont = customer.onts?.[0];
    if (ont) {
        form.name = ont.name || '';
        form.serial_number = ont.serial_number || '';
        form.odp_id = ont.odp_id || '';
        form.olt_id = ont.olt_id || '';
        form.pon_port_id = ont.pon_port_id || '';
        form.ont_id_number = ont.ont_id_number || '';
        form.lat = ont.lat || customer.lat || '';
        form.lng = ont.lng || customer.lng || '';
    } else {
        form.lat = customer.lat || '';
        form.lng = customer.lng || '';
    }
});
        ...
        <div class="py-6"><div class="mx-auto max-w-3xl px-4">
            <form @submit.prevent="submit" class="space-y-4 rounded-lg bg-white p-6 shadow">
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div><label class="block text-sm font-medium text-gray-700">Customer</label><select v-model="form.customer_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm sm:text-sm"><option value="">-- None --</option><option v-for="c in customers" :key="c.id" :value="c.id">{{ c.name }}</option></select><p class="mt-1 text-xs text-gray-400">Pilih customer untuk auto-fill data ONT</p></div>
                    <div><label class="block text-sm font-medium text-gray-700">Name / Merk ONT</label><input v-model="form.name" type="text" :class="{ 'border-red-500': form.errors.name }" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm sm:text-sm" /><p v-if="form.errors.name" class="mt-1 text-sm text-red-600">{{ form.errors.name }}</p></div>
                    <div><label class="block text-sm font-medium text-gray-700">Serial Number</label><input v-model="form.serial_number" type="text" :class="{ 'border-red-500': form.errors.serial_number }" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm sm:text-sm" /><p v-if="form.errors.serial_number" class="mt-1 text-sm text-red-600">{{ form.errors.serial_number }}</p></div>
