...
const initialCustomerId = props.ont.customer_id;
...
watch(() => form.customer_id, (id) => {
    if (!id || id == initialCustomerId) return;
    const customer = props.customers.find(c => c.id == id);
    if (!customer) return;
    const ont = customer.onts?.[0];
    if (ont) {
        form.name = ont.name || form.name;
        form.serial_number = ont.serial_number || form.serial_number;
        form.odp_id = ont.odp_id || form.odp_id;
        form.olt_id = ont.olt_id || form.olt_id;
        form.pon_port_id = ont.pon_port_id || form.pon_port_id;
        form.ont_id_number = ont.ont_id_number || form.ont_id_number;
        form.lat = ont.lat || customer.lat || form.lat;
        form.lng = ont.lng || customer.lng || form.lng;
    } else {
        form.lat = customer.lat || form.lat;
        form.lng = customer.lng || form.lng;
    }
});
