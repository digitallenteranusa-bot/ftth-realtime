        ...
        <div class="py-6"><div class="mx-auto max-w-3xl px-4">
            <form @submit.prevent="submit" class="space-y-4 rounded-lg bg-white p-6 shadow">
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div><label class="block text-sm font-medium text-gray-700">Name *</label><input v-model="form.name" type="text" :class="{ 'border-red-500': form.errors.name }" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm sm:text-sm" /><p v-if="form.errors.name" class="mt-1 text-sm text-red-600">{{ form.errors.name }}</p></div>
                    <div><label class="block text-sm font-medium text-gray-700">ODC *</label><select v-model="form.odc_id" :class="{ 'border-red-500': form.errors.odc_id }" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm sm:text-sm"><option value="">-- Select ODC --</option><option v-for="odc in odcs" :key="odc.id" :value="odc.id">{{ odc.name }}</option></select><p v-if="form.errors.odc_id" class="mt-1 text-sm text-red-600">{{ form.errors.odc_id }}</p></div>
                    <div><label class="block text-sm font-medium text-gray-700">Latitude *</label><input v-model="form.lat" type="number" step="any" :class="{ 'border-red-500': form.errors.lat }" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm sm:text-sm" /><p v-if="form.errors.lat" class="mt-1 text-sm text-red-600">{{ form.errors.lat }}</p></div>
                    <div><label class="block text-sm font-medium text-gray-700">Longitude *</label><input v-model="form.lng" type="number" step="any" :class="{ 'border-red-500': form.errors.lng }" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm sm:text-sm" /><p v-if="form.errors.lng" class="mt-1 text-sm text-red-600">{{ form.errors.lng }}</p></div>
