    ...
    <AuthenticatedLayout>
        ...
        <div class="py-6"><div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            <div class="mt-4 flex justify-center" v-if="odps.links">
                <Link v-for="link in odps.links" :key="link.label" :href="link.url || '#'" class="mx-1 rounded px-3 py-1 text-sm" :class="link.active ? 'bg-blue-600 text-white' : 'bg-white text-gray-700'" v-text="pl(link.label)" />
            </div>
        </div></div>
    </AuthenticatedLayout>
</template>
