...
const props = defineProps({ mikrotik: Object });

const form = useForm({
    name: props.mikrotik.name, host: props.mikrotik.host, api_port: props.mikrotik.api_port,
    api_username: props.mikrotik.api_username, api_password: '',
    snmp_community: props.mikrotik.snmp_community, snmp_port: props.mikrotik.snmp_port,
    is_active: props.mikrotik.is_active, location: props.mikrotik.location || '',
    lat: props.mikrotik.lat || '', lng: props.mikrotik.lng || '', notes: props.mikrotik.notes || '',
});

function submit() {
    form.put(route('mikrotiks.update', props.mikrotik.id));
}
</script>

<template>
    <Head title="Edit Mikrotik" />
    <AuthenticatedLayout>
        <template #header>
            <h2 class="text-xl font-semibold leading-tight text-gray-800">Edit Mikrotik: {{ mikrotik.name }}</h2>
        </template>
        <div class="py-6">
            <div class="mx-auto max-w-3xl px-4">
                <form @submit.prevent="submit" class="space-y-4 rounded-lg bg-white p-6 shadow">
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700">API Password (leave blank to keep)</label>
                    ...
                    <div class="flex justify-end gap-3">
                        ...
                        <button type="submit" :disabled="form.processing" class="rounded-md bg-blue-600 px-4 py-2 text-sm text-white hover:bg-blue-700 disabled:opacity-50">Update</button>
                    </div>
                </form>
            </div>
        </div>
